...
To train and evaluate our model, we collect the logit distributions per index combination, and average them into mean probability distributions by first applying a softmax over the entity group indices, and then take taking the mean. We calculate the loss using Jensen-Shannon divergence (chosen for its symmetry), and show measure performance using Jensen-Shannon similarity (JSS), its complement, which ranges from 0 to 1. We also compare our model to the prevailing view (one-hot distribution at $i_P$), as well as various ablations of our model. We also compare our model to an oracle variant, an upper bound where we replace the learned gaussian with the actual collected probabilities.
